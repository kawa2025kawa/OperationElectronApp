// electron/handlers/tempomaticHandlers.ts
...
interface TempomaticUploadArgs {
  filePaths?: string[];
  expireDate?: string;
}
...
export function setupTempomaticHandlers(): void {
  ipcMain.handle(
    ...
    async (_event, args: TempomaticUploadArgs | undefined) => {
      ...
      if (!expireDate) {
        throw new Error(
          "Tempomaticアップロードの有効期限が指定されていません。",
        );
      }
